...
contract Auction_repo_english is ERC1155MockReceiver // , Interface_to_vault
{
...
//	mapping (address => mapping(uint256 => Auction_info_batch  ) ) _batch_hashid_auction_info ;
	...
	function get_batch_hashid ( address _holder  // polymo
		...
		return keccak256(abi.encode ( _holder ,_target_contract , _target_item_id , _amount , _expiry ) ) ;
	}
...
//		, address _user_proxy_registry
  ...
  function _asSingletonArray(uint256 element) private pure returns (uint256[] memory) {
	...
	function mint_begin_simple_and_bid (
		...
		, uint256 _target_item_id // address
		, uint256 _amount
		, uint256 _offerprice
		, uint _starting_time
		, uint _expiry
		...
		bytes32 batch_hashid = get_batch_hashid ( _holder , _target_contract , _target_item_id , _amount , _expiry ) ;
		if (_is_batch_hashid_onauction[ batch_hashid ] ){ // mint + register done

		} 
	}
	function begin_auction_simple (
		address _target_contract
		, address _holder
		, uint256 _target_item_id // address
		, uint256 _amount
		, address _paymenttoken
		, uint256 _offerprice // starting price
		, uint _starting_time
		, uint _expiry
		, uint _referer_feerate
		, bytes calldata _calldata
		...
		bytes32 batch_hashid = get_batch_hashid ( _holder , _target_contract , _target_item_id , _amount , _expiry ) ;
		require ( _is_batch_hashid_onauction[ batch_hashid]==false , "ERR() already on auction" );
		require ( validate_auction_open (_target_contract , msg.sender , _holder , _asSingletonArray( _target_item_id) ,_asSingletonArray(_amount) ) , "ERR() req invalid");
//		uint256 
		IERC1155 ( _target_contract ).safeTransferFrom ( _holder 
			, address(this)
			, _target_item_id
			, _amount
			, "0x00"
		...
		_batch_hashid_auction_info [ batch_hashid ] = Auction_info_batch (
			...
			, _holder
			, _target_contract
			, _asSingletonArray( _target_item_id ) 
			, _asSingletonArray( _amount )
			, uint256( Auction_instance_status.Active ) 
			, _paymenttoken
			, _offerprice
			, _starting_time
			, _expiry
...
//		, address _user_proxy_registry
		...
		bytes32 batch_hashid = get_batch_hashid(_holder ,_target_contract , _target_item_ids , _amounts );
		require ( _is_batch_hashid_onauction[batch_hashid]==false , "ERR() batch already on auction" ) ;
		require ( validate_auction_open(_target_contract
//		, _user_proxy_registry
			, msg.sender
			, _holder
			, _target_item_ids 
			, _amounts ) , "ERR() auction request not valid" );
		uint256 count = _target_item_ids.length ;
//		for (uint256 i =0; i<count ; i++) {
			IERC1155 ( _target_contract ).safeBatchTransferFrom (_holder
				, address(this)
				, _target_item_ids
				, _amounts
				, _calldata
	...
	//	} //  public constant		
		...
		_is_batch_hashid_onauction[ batch_hashid ] = true ;
		return true ;
	} // end function begin_auction_batch
...
//		bytes32 batch_hashid = get_batch_hashid( _holder , _target_contract , _target_contract ,  );
...
//			if( _bidamount >= _amount){}
	//		else {revert("ERR() price not met"); }
		...
		// extend_e xpiry_on_bid ( _batch_hashid , block.timestamp ) ;
		return extend_expiry_on_bid ( auction_info , _batch_hashid , block.timestamp ) ;
	} // end function _bid

	function bid ( address _target_contract
		, bytes32 _batch_hashid 
		, address _paymenttoken
		, uint256 _amount
		, address _to // beneficiary so that call syntax/context is more flexible
...
// access restriction is none/minimal here so that any caller who can take corresponding gas fee could call
		Auction_info_batch memory auction_info = _batch_hashid_auction_info [ _batch_hashid ] ;
		require( auction_info._expiry <= block.timestamp , "ERR() auction not closeable yet" );
...
/**							emit DepositToVault (
		...
		public 		
	{
		...
		address holder = auction_info._holder ;
		require (  msg.sender == auction_info._opener || msg.sender == IProxyRegistry( _user_proxy_registry).proxies( holder ) , "ERR() not privileged" );
...
/**	function begin_auct ion_single (
		address _target_contract ,
		address _target_item_id ,
		uint256 _amount ,
		address _paymenttoken ,
		uint256 _offerprice
		, uint _starting_time
		...
		require( _is_contract_item _onauction[ _target_contract][_target_item_id]==false , "ERR() item already on auction" ) ;
		
		require( msg.sender ==  )
		_is_contract_item_o nauction[ _target_contract][_target_item_id] = true ;
		_auction_info_single [_target_contract][_target_item_id] = Auction_info_single (			
			, 
		) ;
	} */
/**	struct Auction_info_single {
		address _opener ; // the one who opened this auction
		address _holder ; 
		address _target_contract ; //
		address _target_item_id ;
		uint256 _amount ;
		uint _status ; // 0 : inactive/close , 1 : active , 2:?
		address _paymenttoken ;
		uint256 _offerprice ; 
		uint _starting_time ; 
		uint _expiry ;
